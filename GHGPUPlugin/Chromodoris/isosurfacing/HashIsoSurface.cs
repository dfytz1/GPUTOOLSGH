 ...
 *
 * C# port from ChromodorisGH by Cameron Newnham (GPL-3.0)
...
namespace GHGPUPlugin.Chromodoris
{
    public class HashIsoSurface
    {
        ...
        private Dictionary<int, int> edgeVertices;
        ...
        public Mesh ComputeSurfaceMesh(double iso, ref Mesh mesh)
        {
            isoValue = iso;
                    ...
                    for (int x = 0; x < resX1; x++)
                    {
                        ...
                        if (cellIndex > 0 && cellIndex < 255)
                        {
                            ...
                            if (edgeFlags > 0 && edgeFlags < 255)
                            {
                                ...
                                if ((edgeFlags & 2) > 0)
                                {
                                    double t = isoDiff / (volume.getVoxelAt(offset + resX) - offsetData);
                                    edgeVertices[edgeOffsetIndex + 1] = mesh.Vertices.Add(x, offsetY + t, z);
                                }

                                if ((edgeFlags & 4) > 0)
                                {
                                    double t = isoDiff / (volume.getVoxelAt(offset + sliceRes) - offsetData);
                                    edgeVertices[edgeOffsetIndex + 2] = mesh.Vertices.Add(x, y, offsetZ + t);
                                }
                            }
                        }
                        offsetX++;
        ...
        private void CreateFacesForSlice(Mesh mesh, int z)
        {
            int[] face = new int[16];
            int sliceOffset = sliceRes * z;
        ...
        protected int GetCellIndex(int x, int y, int z)
        {
            ...
            if (volume.getVoxelAt(idx) < isoValue)           cellIndex |= 0x01;
            if (volume.getVoxelAt(idx + sliceRes) < isoValue) cellIndex |= 0x08;
            if (volume.getVoxelAt(idx + resX) < isoValue)    cellIndex |= 0x10;
            if (volume.getVoxelAt(idx + resX + sliceRes) < isoValue) cellIndex |= 0x80;

            idx++;
            if (volume.getVoxelAt(idx) < isoValue)           cellIndex |= 0x02;
            if (volume.getVoxelAt(idx + sliceRes) < isoValue) cellIndex |= 0x04;
            if (volume.getVoxelAt(idx + resX) < isoValue)    cellIndex |= 0x20;
            if (volume.getVoxelAt(idx + resX + sliceRes) < isoValue) cellIndex |= 0x40;
        ...
        public void Reset()
        {
            edgeVertices = new Dictionary<int, int>(resX * resY * 10);
        }
    }
}
