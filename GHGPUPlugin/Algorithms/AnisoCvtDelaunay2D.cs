...
/// <summary>2D Delaunay (Bowyer–Watson) in a local UV plane; outputs triangle vertex indices.</summary>
public static class AnisoCvtDelaunay2D
{
    ...
    /// <summary>Circumradius of triangle in 3D using edge lengths and cross product area.</summary>
    public static double Circumradius(Point3d a, Point3d b, Point3d c)
    {
        double lab = a.DistanceTo(b);
        double lbc = b.DistanceTo(c);
        double lca = c.DistanceTo(a);
        var ab = b - a;
        var ac = c - a;
        double area2 = Vector3d.CrossProduct(ab, ac).Length;
        if (area2 < 1e-30)
